...
from ffmcp.render.detail import LineupAdvice, build_lineup_advice
...
def _swing_player(mine: Lineup, theirs: Lineup) -> tuple[Player | None, float]:
    """The single highest-variance starter across both lineups: the player whose outcome
    swings this specific matchup the most, under the same per-position sd model
    ``domain.simulate`` uses for the season simulation."""
    best_player: Player | None = None
    best_sd = -1.0
    for player in (*mine.started, *theirs.started):
        points = projected_points(player) or 0.0
        sd = player_score_sd(player, points)
        if sd > best_sd:
...
def register_optimize_lineup(mcp: MCPServer) -> None:
    @mcp.tool(
        ...
        description="Find the highest-scoring legal lineup and the swaps to get there.",
        annotations=_READ_ONLY,
    )
    async def optimize_lineup(ctx: Context, week: int | None = None) -> LineupAdvice:
        with adapt_errors():
...
def register_analyze_matchup(mcp: MCPServer) -> None:
    ...
    async def analyze_matchup(
        ...
        with adapt_errors():
            ...
            my_model = lineup_score_model(team_id, my_lineup)
            opp_model = lineup_score_model(opponent_id, opp_lineup)
            ...
            swing_player, swing_sd = _swing_player(my_lineup, opp_lineup)

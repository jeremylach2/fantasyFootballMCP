...
def apply_offer(
    ...
    while len(kept) > len(roster):
        ...
        victim = min(
            droppable, key=lambda p: (projected_points(p, projections) or 0.0, p.player_id)
        )
...
def evaluate(
    ...
    *,
    ...
    my_delta, my_drops = _side_delta(mine, offer.give, offer.get, slots, projections)
    their_delta, their_drops = _side_delta(theirs, offer.get, offer.give, slots, projections)
    return TradeEvaluation(
        offer=offer,
        my_value_delta=round(my_delta * weeks_remaining, 4),
        partner_value_delta=round(their_delta * weeks_remaining, 4),
